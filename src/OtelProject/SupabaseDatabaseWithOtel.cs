...
app.MapPost("/identity/users", async (UserDto request) =>
{
    ...
    using var insertCommand = new NpgsqlCommand("INSERT INTO users (name, email, password) VALUES (@name, @email, @password) RETURNING user_id", connection); 
...
app.MapPut("/identity/users/{id}", async (Guid id, UserDto request) =>
{
    ...
    using var updateCommand = new NpgsqlCommand("UPDATE users SET name = @name, email = @email, password = @password WHERE user_id = @id", connection);
...
app.MapGet("/identity/users", async () =>
{
    ...
    using var selectCommand = new NpgsqlCommand("SELECT * FROM users", connection);
